<?php
	if (isset($_POST['apa']) && $_POST['apa'] != "") {
		...
		include 'modules/model/class.target_akhir.php';
		$data = new target_akhir();
		
		switch ($_POST['apa']) {
			case "get-target":
				$collect = array();
				
				if ($query = $data->get_target_akhir()) {
					while ($rs = $query->fetch_array()) {
						$detail = array();
						array_push($detail, $rs["target_akhir"]);
						array_push($detail, "<button type='button' class='btn btn-sm btn-primary' id='btn-ubah-data' data-id='".$rs["id"]."' data-target='".$rs["target_akhir"]."'>
